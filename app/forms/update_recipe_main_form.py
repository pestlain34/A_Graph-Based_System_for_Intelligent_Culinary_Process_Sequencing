...
class Main_data_of_recipe_form_update(FlaskForm):
    ...
    image = FileField(
        'Фото блюда',
        validators=[FileAllowed(list(ALLOWED_EXTENSIONS), 'Только изображения'), Optional()]
    )

    go_next = SubmitField('Сохранить изменения')
